...
const isValid = computed(() => {
  ...
  return price.value && !isNaN(priceNum) && priceNum >= 0;
});
...
const handleSetPrice = async () => {
  ...
  try {
    ...
    step.value = 'success';
    emit('success', `Animal price set to ${price.value} SOL`);
      ...
      <div v-if="modelValue" @click="close"
        ...
        <div @click.stop class="bg-white rounded-2xl shadow-2xl max-w-md w-full animate-scale-in">
          ...
          <form @submit.prevent="handleSetPrice" class="p-6 space-y-5">
            ...
            <div>
              <label for="price" class="block text-sm font-medium text-brand-text-light mb-1.5">
                Price (SOL) *
              </label>
              <div class="relative">
...
.button-primary {
  @apply inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm hover:opacity-90 transition-opacity focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed;
}
